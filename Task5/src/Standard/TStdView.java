...
public   class  TStdView  extends JFrame  implements ActionListener, IView {
	...
	public TStdView(ICalculator calculator)
	{
	 ...
	 private void  setGUI__wrappee__BasisOperationen  (boolean isHistory)
	{
		int width = 400;
		int height = 300;
		...
		_bnClose = new JMenuItem("Schließen");

		// Untermenueeinträge hinzufuegen
		...
		*/
		...
		// virtuelles Panel erstellen für die Anordnung der Buttons
		...
		if (isHistory)
		{
			...
			contentPane.add(_historyDisplay, new GridBagConstraints(0, cpY++, 5, 1, 100.0, 100.0,
				GridBagConstraints.NORTH, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		}

		contentPane.add(_display, new GridBagConstraints(0, cpY++, 5, 1, 100.0, 100.0, 
			GridBagConstraints.NORTH, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		...
		contentPane.add(_bnNextVar, new GridBagConstraints(2, cpY, 1, 1, 100.0, 100.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(insetSize, insetSize, insetSize, insetSize), 0, 0));
		
		contentPane.add(_bnGleich, new GridBagConstraints(3, cpY, 1, 1, 100.0, 100.0, 
			GridBagConstraints.CENTER, GridBagConstraints.BOTH, 
			new Insets(insetSize, insetSize, insetSize, insetSize), 0, 0));
		...
		if (extPanel != null)
		{
			contentPane.add(extPanel, new GridBagConstraints(4, cpY, 1, 2, 100.0, 100.0, 
					GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 1, 1, 1), 0, 0));
		}
		
		contentPane.add(buttons, new GridBagConstraints(0, ++cpY, 4, 1, 100.0, 100.0, 
	...
	private void setGUI(boolean isHistory)
	{
		...
		setGUI__wrappee__BasisOperationen(isHistory);
	}
	...
	public String getViewName()
	{
	 ...
	 private void  setViews__wrappee__BasisOperationen  (IView[] views)
	{
	...
	{
		setViews__wrappee__BasisOperationen(views);
	...
	// private Methode zum schließen des Programms
	...
	{
		_display.setText(_display.getText() + text);
	}

	
	
	protected void historyAppend(int value)
	{
		if (_historyDisplay != null)
		{
			_historyDisplay.setText(_historyDisplay.getText() + value);
		}
	}

	
	
	protected void historyAppend(String text)
	{
		if (_historyDisplay != null)
		{
			_historyDisplay.setText(_historyDisplay.getText() + text);
		}
	}
    ...
    {
    	System.out.println("onInputOperation: " + op);
    	...
    	else if (_calculator.onOperation(op))
    	{
    		String text = _display.getText();
	    	...
	    	if (_historyDisplay != null)
			{
	    		_historyDisplay.setText(op + "(" + text + "; ");
			}
